...
class AttributeList(object):
    def __init__(self, features, bin_structure):
        ...
        self.attribute_list = [ np.empty((self.dataset_size-self.feature_missing_cnt[i], ),
                                         dtype=[("attribute", "uint8"), ("index", "int32")])
    ...
    def construct_attribute_list(self):
        for i in range(self.feature_dim): # for each feature
            ...
            bin_example_index = [ [] for _ in range(bin_number)] # 根据每个bin就可以找到example index
            missing_value_index = []

            for index in range(self.dataset_size): # 遍历样本
    ...
    def clean_up(self):
        # clean not necessary instance attribute
